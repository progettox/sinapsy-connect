...
class BrandChatInboxPage extends ConsumerStatefulWidget {
  const BrandChatInboxPage({super.key});

  @override
  ConsumerState<BrandChatInboxPage> createState() => _BrandChatInboxPageState();
}

class _BrandChatInboxPageState extends ConsumerState<BrandChatInboxPage> {
  bool _isLoading = false;
  String? _error;
  List<BrandChatNotificationItem> _items = const <BrandChatNotificationItem>[];
  ...
  Future<void> _loadChats() async {
    ...
    if (brandId == null || brandId.isEmpty) {
      ...
      setState(() {
        _isLoading = false;
        _items = const <BrandChatNotificationItem>[];
    ...
    try {
      final chats = await ref
          ...
          .listBrandChatNotifications(brandId: brandId, limit: 100);
      if (!mounted) return;
      setState(() {
        _isLoading = false;
        _items = chats;
  ...
  Future<void> _openChat(BrandChatNotificationItem item) async {
  ...
  String _usernameFor(BrandChatNotificationItem item) {
                  ...
                  if (_isLoading && _items.isEmpty)
                    ...
                    ..._items.map((item) {
                      return _ChatInboxTile(
                        ...
                        subtitle:
                            item.lastMessage?.trim().isNotEmpty == true
...
class _ChatInboxTile extends StatelessWidget {
  ...
  final BrandChatNotificationItem item;
            ...
            child: ClipOval(
              child: clean.isNotEmpty
                  ? Image.network(
                      clean,
                      fit: BoxFit.cover,
                      errorBuilder: (_, _, _) => _AvatarFallback(letter: letter),
                    )
    ...
    return const Padding(
      ...
      child: Column(
        children: [
          ...
          SizedBox(height: 4),
          Text(
            'Le chat compaiono quando accetti una candidatura.',
            textAlign: TextAlign.center,
            style: TextStyle(
